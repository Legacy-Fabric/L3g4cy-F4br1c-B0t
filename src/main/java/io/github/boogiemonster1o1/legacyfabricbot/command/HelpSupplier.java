...
public interface HelpSupplier {
	...
	static int printAll(Mono<MessageChannel> channelMono) {
		channelMono.flatMap(channel -> channel.createEmbed(spec -> {
			...
			SUPPLIERS.forEach((str, supp) -> {
				spec.addField(str, "`" + supp.getUsage() + "`", true);
			});
		})).subscribe();
	...
	@CanIgnoreReturnValue
	static int printOne(Mono<MessageChannel> channelMono, String command) throws CommandSyntaxException {
		...
		channelMono.flatMap(channel -> channel.createEmbed(spec -> {
			...
			spec.addField("Usage", supp.getUsage(), false);
